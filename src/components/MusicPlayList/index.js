...
class MusicPlayList extends Component {
  ...
  onDeleteClick = id => {
    ...
    const filteredList = trackList.filter(eachItem => eachItem.id !== id)
    this.setState({trackList: filteredList})
  }

  renderNoResultsView = () => <p>No Songs Found</p>

  render() {
    const {searchInput, trackList} = this.state
    const searchedList = trackList.filter(eachTrack =>
